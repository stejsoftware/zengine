...
{
    private static Logger       log        = LoggerFactory.getLogger(StoryDatabase.class);
    ...
    public boolean load_stories(String story_folder)
    {
    ...
    private byte[] load_story(String filename)
    {
        ...
        if( f.canRead() && f.isFile() )
        {
            ...
            {
                log.warn("Error: " + filename + ": " + ex.getMessage());
            }

        }
        else
        {
            log.warn("Error: file not readable: " + filename);
        }
